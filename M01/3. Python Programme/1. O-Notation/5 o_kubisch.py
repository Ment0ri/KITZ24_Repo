...
# Definiere eine Operation mit dreifach geschachtelten Schleifen
def operation(array):
    # Die äußere Schleife iteriert über jedes Element in "array"
    for i in range(len(array)):
        # Die mittlere Schleife iteriert ebenfalls über jedes Element in "array"
        for j in range(len(array)):
            # Die innerste Schleife iteriert auch über jedes Element in "array"
            for k in range(len(array)):
                array[i] += array[j] * array[k]  # Multipliziere jedes Paar (j, k) und addiere das Ergebnis zu array[i]
...
k = 2**15                    # Obergrenze für "n" (32.768), definiert das Ende der Schleife
...
while n <= k:
    ...
    operation(array)             # Führe die dreifach-geschachtelte Operation auf dem Array aus
